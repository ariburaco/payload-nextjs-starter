...
const HeroBanner = ({
  ...
  settings,
  blockIndex,
}: HeroBannerBlock & AdditionalBlockProps) => {
  const imageSrc =
    typeof backgroundImage === "object" ? backgroundImage?.url : null;
  const invertBackgroundImageInDarkMode = (settings || []).includes(
    "invertBackgroundImageInDarkMode",
  );
      ...
      <div className="relative z-10 flex grow items-center">
        <Container>
          <div className="space-y-6 md:mb-20">
            {isNotNull(preTitle) && (
              <span className="inline-block rounded-md text-sm font-semibold dark:text-zinc-500">
                {preTitle}
              </span>
            )}
            {isNotNull(title) && (
              <h2 className="max-w-xl text-4xl font-extrabold tracking-tight dark:text-zinc-100 sm:text-5xl md:text-6xl">
                {title}
              </h2>
            )}
            {isNotNull(subtitle) && (
              <p className="mt-4 max-w-2xl text-2xl text-zinc-700 dark:text-zinc-500">
                {subtitle}
              </p>
            )}
          </div>
