...
export const isComponentMatching = (
  ...
  if (typeof window === "undefined") {
    ...
    displayName = Array.isArray(child?.type?._payload?.value)
      ...
      : child.type?._payload?.value?.displayName;
  } else {
    // @ts-ignore
    displayName = Array.isArray(child?.type?._payload?.value)
      ? // @ts-ignore
        child.type?._payload?.value?.at(-1)
      : // @ts-ignore
        child?.type?._payload?.value?.name ||
        child?.type?._payload?.value?.displayName;
  }
...
export function findComponentsOfType(
  ...
  const foundComponents = React.Children.toArray(children).filter(
    (child) =>
      React.isValidElement(child) && isComponentMatching(child, componentType),
  );
...
export function findComponentOfType(
  children: React.ReactNode,
  componentType: React.ElementType,
...
export function applyPropsToChildrenOfType(
  ...
  return React.Children.map(children, (child, index) => {
    ...
    const props = isEligibleComponent
      ? { ...extraProps, ...(includeIndex ? { index } : {}) }
      : {};
    ...
    const childProps = {
      ...
      ...props,
      // @ts-ignore
      children: applyPropsToChildrenOfType(
